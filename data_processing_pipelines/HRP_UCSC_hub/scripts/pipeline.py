...
Steps:
  ...
  2. Convert split BED files to bigBed (per-platform), plus tiered consensus
     bed (concensus/consensus_tiered.bed) to bigBed (per RNA type)
...
# Source BED files from each sequencing modality, merged across all RNA types.
# Replace filenames with your actual input files.
SOURCE_DIR = "final_bedRmods"

COMBINED_FILES = {
    "SRS": os.path.join(SOURCE_DIR, "SRS_modifications.bed"),
    "LRS": os.path.join(SOURCE_DIR, "LRS_modifications.bed"),
    "MS":  os.path.join(SOURCE_DIR, "MS_modifications.bed"),
}

# Combined tiered consensus bed (all RNA types, header: chr/start/end/name/tier/strand).
TIERED_COMBINED_FILE = os.path.join("concensus", "consensus_tiered.bed")
...
GITHUB_USER  = "[GITHUB_USER]"   # replace with your GitHub username
GITHUB_REPO  = "[GITHUB_REPO]"   # replace with your hub repository name
HUB_BASE_URL = f"https://raw.githubusercontent.com/{GITHUB_USER}/{GITHUB_REPO}/main"
PAPER_URL    = "PLACEHOLDER_DOI"  # replace with published DOI/URL
...
# RNA sequence track (bigPSL) — file too large for GitHub, hosted on cloud storage.
# Replace [GCS_BUCKET] with your GCS bucket name and adjust the filename as needed.
BIGPSL_GCS_URL   = "https://storage.googleapis.com/[GCS_BUCKET]/reads.bigPsl.bb"
DECORATOR_BIGBED = "decorator.bigBed"
...
""")
        stanzas.append(
            ...
            "bigDataUrl https://storage.googleapis.com/[GCS_BUCKET]/NA12878_WGS_variants.vcf.gz\n"
...
def main():
    ...
    print(f"  3. Upload reads.bigPsl.bb to cloud storage and update BIGPSL_GCS_URL")
