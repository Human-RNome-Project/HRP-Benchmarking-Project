...
workflow ont_mRNA_pilot_merge_modkit_grid {
    input {
        Array[File] bamfiles
...
task ModkitPileup {
    ...
    modkit pileup \
        ~{bam} \
        "${filename}.threshold_~{threshold}.genome.bed" \
        --ref ~{reference} \
        --threads ~{cpus} \
        ...
        --log-filepath "${filename}.threshold_~{threshold}.genome.log" \
        --bedrmod
    >>>

    output {
        File bed = basename(bam, ".bam") + ".threshold_~{threshold}.genome.bed"
        File log = basename(bam, ".bam") + ".threshold_~{threshold}.genome.log"
    }
