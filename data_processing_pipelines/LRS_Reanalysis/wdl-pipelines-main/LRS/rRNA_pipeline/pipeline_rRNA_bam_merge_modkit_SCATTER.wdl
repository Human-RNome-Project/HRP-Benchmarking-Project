...
workflow ont_rRNA_merge_modkit_grid {
    input {
        Array[File] bamfiles
...
task ModkitPileup {
    ...
    modkit pileup \
        ~{bam} \
        "${filename}.threshold_~{threshold}.rRNA.bed" \
        --ref ~{reference} \
        --threads ~{cpus} \
        ...
        --log-filepath "${filename}.threshold_~{threshold}.rRNA.log" \
        --bedrmod
    >>>

    output {
        File bed = basename(bam, ".bam") + ".threshold_~{threshold}.rRNA.bed"
        File log = basename(bam, ".bam") + ".threshold_~{threshold}.rRNA.log"
    }
