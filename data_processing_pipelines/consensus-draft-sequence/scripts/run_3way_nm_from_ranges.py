#!/usr/bin/env python3
"""
rRNA 3-way grid search for Nm mods using quantile grids from parameter ranges TSV.
...
# ── Run 3-way searches ──────────────────────────────────────────────────────
print("\n" + "=" * 70)
print("3-WAY GRID SEARCH for Nm mods (from parameter ranges TSV)")
print("=" * 70)
...
if best_results:
    ...
    df.to_csv(OUTDIR / "rRNA_3way_Nm_best.tsv", sep="\t", index=False)
    print(f"\nSaved rRNA_3way_Nm_best.tsv ({len(df)} rows)")
else:
