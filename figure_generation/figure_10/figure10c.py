...
def build_sprinzl_mapping(df:pd.DataFrame, id_col="tRNA_name"):
    ...
    return result
#sprinzl anotated heatmap of tRNA sequences
...
def create_data_matrix(positions:list,df_cons:pd.DataFrame,sprinzl_dict: dict,tRNAs:list,sprinzl_df:pd.DataFrame)->pd.DataFrame:
    #print(df_cons[df_cons["name"]=="ms2i6A"])
    ...
    # fill matrix
    # 0 = no mod,ns=no sequence, (X,tier)= modification
...
def plot_heatmap(data_matrix: pd.DataFrame,present_mods:list):
    ...
    plt.tight_layout()
    plt.savefig("tRNA_Illumina_heatmap.svg")
    plt.show()


############# Plotting #######################
consensus = "/home/alex/Documents/PHD/RNOME/Paper/Illumina/Illumina_combined_polyARNA_tRNA_rRNA.bed"

df_cons = pd.read_csv(consensus, sep="\t", comment='#',header=None)
df_cons.columns =["chrom","chromStart","chromEnd","name","strand","score","thickStart","thickEnd","itemRgb","coverage","frequency","single_letter_code","mod_id"]
df_cons = df_cons[df_cons["chrom"].str.startswith("hs_tRNA")]
df_cons = transform_tRNA_name(df_cons)

#remove hs_tRNALeu_TAG & "hs_tRNASer_TGA" due to no sprinzl coordinates and only in full refrence not 47 reference
df_cons = df_cons[(df_cons["chrom"] != "hs_tRNALeu_TAG") & (df_cons["chrom"] != "hs_tRNASer_TGA")]
