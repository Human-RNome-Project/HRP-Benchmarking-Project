...
def plot_28S(overlap_multi_s, overlap_single, non_overlap_multi_s, non_overlap_single, frag_dict):
    # =========================
    ...
    ax_frag.set_xlabel("28S Position", fontsize=label_fontsize)
    ...
    ax_mod.set_xticks(np.arange(0, xmax + 1, 500))
    ...
    m66A = Patch(facecolor=MOD_COLORS["m6,6A"], label="m6,6A")
    ...
    plt.tight_layout()
    plt.savefig("rRNA_28S_modifications.svg", bbox_inches="tight")
    plt.show()
...
def main():
    ...
    # Find overlaps
    overlap_multi_s_28S, overlap_single_28S, non_overlap_multi_s_28S, non_overlap_single_28S = reference_overlap(rRNA_detailed_consensus,taoka_ref, "28S")
    

    plot_28S(overlap_multi_s_28S, overlap_single_28S, non_overlap_multi_s_28S, non_overlap_single_28S, frag_dict_28S)
