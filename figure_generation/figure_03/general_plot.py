...
def plot_broken_axis(df_replicas: pd.DataFrame,mean_df:pd.DataFrame, break_point:float, out_name:str, ratio:tuple, type:str, cut_height:tuple):
    mean_df= mean_df[["name","mean","std"]]
    df = df_replicas.merge(mean_df, on="name", how="inner")

    # keep only rows with valid mean
    df = df[df["mean"].notna()]
    ...
    fig, (ax_top, ax_bot) = plt.subplots(
        ...
        gridspec_kw={"height_ratios": [cut_height[0],cut_height[1]], "hspace": 0.05},
    )
    ...
    ax_top.set_ylim(break_point, 77)
    ...
    ax_bot.set_xticks(x)
    ax_bot.set_xticklabels(order, rotation=45, ha="right", fontsize=5.2)
